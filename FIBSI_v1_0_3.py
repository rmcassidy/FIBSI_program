...
class Yseries(object):
    ...
    def exclude_replace_events(self,X,Cut):
        ...
        for e in self.evts:
            add = False
            if Cut.x_c is not None and e.numpts>=Cut.x_c: 
                add = True; self.excluded+=1
            elif Cut.yc_b is not None and e.dir == 'b' and abs(Y[e.ampi])>=abs(Cut.yc_b): 
                add = True; self.excluded+=1
            elif Cut.yc_a is not None and e.dir == 'a' and abs(Y[e.ampi])>=abs(Cut.yc_a): 
                add = True; self.excluded+=1
            if add:
                e.excluded=True
...
def e_formatted_out(X, Y_M, filename,names):
    ...
    for i,Y in enumerate(Y_M):
        for evt in Y.evts:
            s = str(names[i]) + "\t" + str(evt.quad)+ "\t" + str(evt.dir) + "\t" + str(evt.startx) + "\t" + str(evt.endx) + "\t" +  str(evt.ampx) + "\t" + str(evt.dur) + "\t" + str(evt.ampy) + "\t" + str(evt.midx) + "\t" + str(evt.halfdur) + "\t" +str(evt.AUC) + "\t" + "\n"
            F.write(s)
...
def process_file(): 
    ...
    # Load data
    path_filename = os.path.abspath(ARGS.filename) #EDITED
    filename = os.path.basename(path_filename).split('.')[0] #EDITED

    # Load CSV and skip header
    raw_data = np.genfromtxt(path_filename,delimiter=',',dtype=None,encoding=None,skip_header=ARGS.sh)

    # EDIT
    # If length of x_y_raw = 1, this means only the X index was entered and all other columns should be used
    if len(ARGS.c) == 1:
        cols = [n for n in range(0,len(raw_data[0]))]
        cols.pop(ARGS.c[0])
        cols.insert(0,ARGS.c[0])
        ARGS.c = cols
    # /EDIT
...
def set_check_ARGS():
    ...
    ap.add_argument("-c","--columns",
                    ...
                    help="0-indexed location of data. first is x vals, others are y vals. default is 0 1. If only one number for x vals like -c 2 will use all other columns for y vals") #edited
        ...
        if len(ARGS.filt)==0:
            ...
            while len(arg_list)>0:
                ...
                for Y in Y_M:
                    ...
                    Y.dfy = Y.fitY # EDIT to correct error where filtered dfy without normalization showed the raw data
                    Y.log(LogF)
